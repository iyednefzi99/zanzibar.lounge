    ...
    <div className="flex min-h-dvh flex-col bg-night">
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-brass/20 bg-deep/95 px-4 backdrop-blur-sm">
        ...
        <form action="/api/guest/logout" method="post">
          <button
            type="submit"
            className="min-h-[44px] min-w-[44px] rounded-lg px-3 text-xs text-shell-dim transition-colors hover:text-coral"
          >
            {locale === "fr" ? "Quitter" : locale === "ar" ? "خروج" : "Logout"}
          </button>
        </form>
      </header>
